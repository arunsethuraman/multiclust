...
def multiclust(n):
    ...
    result = subprocess.run(["./multiclust", "-f", "SS2alleles001.str", "-o", unique_filename], capture_output=True, text=True)
...
for i in range(20):
    if(len(multiprocessing.active_children()) < len(os.sched_getaffinity(0))):
        for j in range(len(os.sched_getaffinity(0))-len(multiprocessing.active_children())):
            ...
            t = multiprocessing.Process(target=multiclust, args=(j,))
            processes.append(t)
            t.start()
            print(str(len(multiprocessing.active_children())))
            i+=j
